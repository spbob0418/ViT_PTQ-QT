...
parser.add_argument('--prompt-search-epoch', default='None', type=int)
parser.add_argument('--qft-model', default='None', type=str)
parser.add_argument('--ps-model', default='None', type=str)
parser.add_argument('--prompt-searching-lr', type=float, default='None')
parser.add_argument('--qft-lr', type=float, default='None')
...
#                  base_lr_1, base_lr_2, min_lr=0.0, iters_per_epoch=None, last_epoch=-1):
...
#         self.min_lr = min_lr
#         self.iters_per_epoch = iters_per_epoch  # for step_update
#         self._step_update_called = False
#         self.num_updates = 0
#         super().__init__(optimizer, last_epoch)

#     def get_lr(self):
#         # For epoch-based scheduler only
...
def main():
    ...
    for old_key, new_key in key_mapping.items():
        if new_key in ps_model.state_dict():
    ...
    with torch.no_grad():
        extra_tokens_from_model = ps_model.pos_embed[:, old_num_extra_tokens:new_num_extra_tokens, :]

    num_patches = ps_model.patch_embed.num_patches #576
    ...
    ps_model.load_state_dict(new_checkpoint, strict=False)

    model_keys = set(ps_model.state_dict().keys())
    ...
    if args.num_classes is None:
        assert hasattr(ps_model, 'num_classes'), 'Model must have `num_classes` attr if not set on cmd line/config.'
        args.num_classes = ps_model.num_classes  # FIXME handle model default vs config num_classes more elegantly

    if args.grad_checkpointing:
        model.set_grad_checkpointing(enable=True)


    data_config = resolve_data_config(vars(args), model=ps_model, verbose=args.local_rank == 0)
    ...
    if args.split_bn:
        ...
        ps_model = convert_splitbn_model(ps_model, max(num_aug_splits, 2))

    # move model to GPU, enable channels last layout if set
    ps_model.cuda()
    qft_model.cuda()
    if args.channels_last:
        ps_model = ps_model.to(memory_format=torch.channels_last)
    ...
    if args.distributed and args.sync_bn:
        ...
        if has_apex and use_amp == 'apex':
            ...
            ps_model = convert_syncbn_model(ps_model)
        else:
            ps_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(ps_model)
    ...
    if args.local_rank == 0:
        _logger.info(
            f'Model {safe_model_name(args.ps_model)} created, param count:{sum([m.numel() for m in ps_model.parameters()])}')
        _logger.info(f"number of params for requires grad: {sum(p.numel() for p in ps_model.parameters() if p.requires_grad)}")
    ...
    if args.model_ema:
        ...
        model_ema = ModelEmaV2(
            ps_model, decay=args.model_ema_decay, device='cpu' if args.model_ema_force_cpu else None)
    ...
    if args.distributed:
        if has_apex and use_amp == 'apex':
            ...
            ps_model = ApexDDP(ps_model, delay_allreduce=True,  find_unused_parameters=True)
        else:
            if args.local_rank == 0:
    ...
    ################################################################
    ##prompt searching stage
    ################################################################
    print("Prompt Searching Stage Start!")
    for name, param in ps_model.named_parameters():
        param.requires_grad = True

    for name, param in ps_model.named_parameters():
        if param.requires_grad == False:
            print("freezed param in prompt searching stage", name)

    for epoch in range(0, args.prompt_search_epoch):
        ...
        if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
            if args.local_rank == 0:
                _logger.info("Distributing BatchNorm running means and vars")
            distribute_bn(ps_model, args.world_size, args.dist_bn == 'reduce')

        eval_metrics = validate(ps_model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
    ...
    ################################################################
    ## Save ps_model and load into qft_model
    ################################################################
    if args.local_rank == 0:
        print("Prompt Searching Stage Finish!")
        print("ps_model state_dict saving")
        DIR = os.getenv("DIR", "default_model")  # 환경 변수가 없을 경우 기본값 설정
        VERSION = os.getenv("VERSION", "default_version") 
        MODEL = os.getenv("MODEL", "default_version") 
        ps_ckpt_path = os.path.join(f'{DIR}/output/{MODEL}/cifar100/{VERSION}', 'checkpoint_ps.pth')
        torch.save({
            'state_dict': ps_model.state_dict(),
            'model_ema': get_state_dict(model_ema)
        }, ps_ckpt_path)

        # ps_model에서 저장한 state_dict을 qft_model에 로드
        print("State_dict load to qft model")
        checkpoint = torch.load(ps_ckpt_path, map_location='cpu')
        qft_model.load_state_dict(checkpoint['state_dict'], strict=False)
    ...
    ##QFT stage 
    ################################################################
    print("Quantized Finetuning Stage Start!")
    for name, param in qft_model.named_parameters():
        param.requires_grad = True
        if args.freeze_reg:
    ...
    for name, param in qft_model.named_parameters():
        if param.requires_grad == False:
            print("freezed param in QFT stage", name)

    for epoch in range(args.prompt_search_epoch, args.epochs):
        if args.distributed and hasattr(loader_train.sampler, 'set_epoch'):
            loader_train.sampler.set_epoch(epoch)
        train_metrics = train_one_epoch(
        ...
        if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
            if args.local_rank == 0:
                _logger.info("Distributing BatchNorm running means and vars")
            distribute_bn(qft_model, args.world_size, args.dist_bn == 'reduce')

        eval_metrics = validate(qft_model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
