...
parser.add_argument('--lambda-coef', default=0, type=float)
...
def main():
    ...
    elif args.prompt_search_mode_with_custom_loss: 
        model = create_model(
            args.model,
            mode=args.loss_mode,
            a_quant_type = args.a_quant_type,
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            output, QKcosTerm = model(input, epoch = epoch, iteration = batch_idx, device_id=args.device)
            ...
            loss = (1-args.lambda_coef)*loss - args.lambda_coef * QKcosTerm
