...
from .data.dataset_factory import build_dataset, ImageDataset
...
DIST_PORT=7777
WANDB = os.getenv("WANDB", "0")  # 기본값을 "0"으로 설정
WANDB_LOG = WANDB.lower() in ("1", "true", "yes")
...
def get_args_parser():
    ...
    parser.add_argument('--model-freeze', action='store_true')
    parser.add_argument('--qft-mode', type=str)
    parser.add_argument('--reg-freeze', action='store_true')
    ...
    # * Finetuning params
    parser.add_argument('--prompt_search_mode', action='store_true')
    parser.add_argument('--attn-only', action='store_true') 
    parser.set_defaults(prompt_search_mode=False)
    
    # Dataset parameters
    parser.add_argument('--data-path', default='/datasets01/imagenet_full_size/061417/', type=str,
                        help='dataset path')
    parser.add_argument('--dataset', default='IMNET', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19', 'OXFORD_PET','cub2011'],
...
def main(args):
    ...
    dataset_train, args.nb_classes = create_dataset(
        args.dataset, root=args.data_path, split=args.train_split, is_training=True,
        class_map=args.class_map,
        download=dataset_download,
        batch_size=args.batch_size,
        repeats=args.epoch_repeats)
    dataset_eval, _ = create_dataset(
    ...
    if args.resume or args.qft_mode:
        checkpoint_path = args.resume if os.path.exists(args.resume) else args.qft_mode
    ...
    if args.model_freeze: 
        for name, param in model.named_parameters():
            if name in ["module.cls_token", "module.pos_embed", "module.reg_token", 
                ...
                param.requires_grad = False  # Model Freeze (학습 불가능)
    if args.reg_freeze: 
        for name, param in model.named_parameters():
            if name in ["module.reg_token"]:
                param.requires_grad = False  # 학습 가능하게 설정
            else:
                param.requires_grad = True  # Model Freeze (학습 불가능)

    save_cp_point_real = [1000, 2000, 3000, 4000, 20000]
    save_cp_point = [x*4 -1 for x in save_cp_point_real]
    ...
    for training_step in range(args.start_step, args.training_steps):
        ...
        torch.cuda.empty_cache()
        dist.barrier()
        if training_step % (args.accum_steps * 500) == 0 and args.sample_eval:
            if device_id == 1:
