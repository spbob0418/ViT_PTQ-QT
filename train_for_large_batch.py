...
from models import quant_vision_transformer_CushionCache_QKcos, vision_transformer, swin_transformer, convnext, as_mlp
...
def main():
    ...
    if args.evaluate:
        ...
        # if saver is not None and epoch in args.save_cp_point:
...
def train_one_epoch(
    ...
    model.train()
    optimizer.zero_grad()  # ✅ 초기화 위치 유지

    end = time.time()
    last_idx = len(loader) - 1
    num_updates = epoch * len(loader)

    accumulated_loss = 0  # ✅ 누적 손실 변수
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            loss = loss / args.accum_step  # ✅ 손실 스케일링 추가

        if not args.distributed:
            losses_m.update(loss.item() * args.accum_step, input.size(0))  # ✅ 원래 scale로 저장

        accumulated_loss += loss  # ✅ 스케일링된 손실을 누적

        # ✅ 그래디언트 누적
        if loss_scaler is not None:
            loss_scaler(
                accumulated_loss, optimizer,
                clip_grad=args.clip_grad, clip_mode=args.clip_mode,
                parameters=model_parameters(model, exclude_head='agc' in args.clip_mode),
                create_graph=second_order)
        else:
            accumulated_loss.backward(retain_graph=False if (batch_idx + 1) % args.accum_step == 0 else True)

        # ✅ Accumulation Step마다 optimizer 업데이트
        if (batch_idx + 1) % args.accum_step == 0 or last_batch:
            if args.clip_grad is not None:
                dispatch_clip_grad(
                    model_parameters(model, exclude_head='agc' in args.clip_mode),
                    value=args.clip_grad, mode=args.clip_mode)
            optimizer.step()
            optimizer.zero_grad()  # ✅ 매 update 후 초기화
            accumulated_loss = 0  # ✅ 누적 손실 초기화
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                losses_m.update(reduced_loss.item() * args.accum_step, input.size(0))
