...
class Attention(nn.Module):
    ...
    def forward(self, x, epoch, iteration, device_id, prefix_token_num):
        ...
        # 1. q_all vs k_reg
        B, H, Tq1, D = q_all.shape
        _, _, Tk1, _ = k_reg.shape

        q1_flat = q_all.reshape(B * H, Tq1, D)
        k1_flat = k_reg.reshape(B * H, Tk1, D)

        q1_norm = F.normalize(q1_flat, dim=-1)
        k1_norm = F.normalize(k1_flat, dim=-1)

        cos1 = torch.bmm(q1_norm, k1_norm.transpose(1, 2))  # (B*H, Tq1, Tk1)

        # 2. q_reg vs k_all
        Tq2 = q_reg.shape[2]
        Tk2 = k_all.shape[2]

        q2_flat = q_reg.reshape(B * H, Tq2, D)
        k2_flat = k_all.reshape(B * H, Tk2, D)

        q2_norm = F.normalize(q2_flat, dim=-1)
        k2_norm = F.normalize(k2_flat, dim=-1)

        cos2 = torch.bmm(q2_norm, k2_norm.transpose(1, 2))  # (B*H, Tq2, Tk2)

        # 3. concat 후 mean
        QKcos_total = torch.cat([cos1.flatten(), cos2.flatten()], dim=0)
        QKcos_mean = QKcos_total.mean()
        ...
        if device_id == 'cuda:0':
            if iteration == 0: 
                ...
                save_dir = f"./output/{MODEL}/cifar100/{VERSION}/QKAttentionMap"
...
class Q_Block(nn.Module):
    ...
    def forward(self, x, is_index_eval, epoch, iteration, device_id, prefix_token_num):
        residual_1 = x
        x = self.norm1(x)
        x, QKcos = self.attn(x, epoch, iteration, device_id, prefix_token_num)
        x = residual_1 + x
...
class CustomSequential(nn.Module):
    def __init__(self, mode, depth, *modules):
    ...
    def forward(self, x, is_index_eval, epoch, iteration, device_id, prefix_token_num):
        ...
        for module in self.modules_list:
            x, QKcos = module(x, is_index_eval, epoch, iteration, device_id, prefix_token_num)
            QKcos_sum += QKcos

        if self.mode == 'mean': 
            QKcosTerm = QKcos_sum / self.depth
        elif self.mode == 'sum': 
            QKcosTerm = QKcos_sum
        return x, QKcosTerm
...
class lowbit_VisionTransformer(VisionTransformer):
    def __init__(self, mode, a_quant_type, ag_quant_type, register_num, num_classes, abits, wbits, w_gbits, a_gbits,
        ...
        self.blocks = CustomSequential(mode, depth, *[
    ...
    def forward_features(self, x, is_index_eval, epoch, iteration, device_id):
        ...
        x, QKcosTerm = self.blocks(x, is_index_eval, epoch, iteration, device_id, self.prefix_token_num)

        if is_index_eval:
            return x
        
        x = self.norm(x)
        return x[:, 0], QKcosTerm

    def forward(self, x, is_index_eval=False, epoch=None, iteration=None, device_id=None):
        x, QKcosTerm = self.forward_features(x, is_index_eval, epoch, iteration, device_id)
...
@register_model
def fullbits_vit_base_patch16_224(pretrained=False, mode=None, a_quant_type=None, ag_quant_type=None, register_num=0, num_classes=0, **kwargs):
...
@register_model
def fourbits_vit_base_patch16_224(pretrained=False, mode=None, a_quant_type=None, ag_quant_type=None, egister_num=0, num_classes=0, **kwargs):
    model = lowbit_VisionTransformer(
        mode = mode,
        a_quant_type=a_quant_type,
        ag_quant_type=ag_quant_type,
...
@register_model
def full_eightbits_vit_base_patch16_224(pretrained=False, mode=None, a_quant_type=None, ag_quant_type=None, register_num=0, num_classes=0, **kwargs):
    model = lowbit_VisionTransformer(
        mode = mode,
        a_quant_type=a_quant_type,
        ag_quant_type=ag_quant_type,
...
@register_model
def eightbits_vit_base_patch16_224(pretrained=False, mode=None, a_quant_type=None, ag_quant_type=None, register_num=0, num_classes=0, **kwargs):
    model = lowbit_VisionTransformer(
        mode = mode,
        a_quant_type=a_quant_type,
        ag_quant_type=ag_quant_type,
...
@register_model
def fourbits_vit_small_patch16_224(pretrained=False, mode=None, a_quant_type=None, ag_quant_type=None, register_num=0, num_classes=0, **kwargs):
    model = lowbit_VisionTransformer(
        abits=4, wbits=4, w_gbits=None, a_gbits=4,
