...
def rename_keys(checkpoint, is_transfer_learning):
    ...
    for i, old_key in enumerate(checkpoint.keys()):  # 루프 카운트 추가
        ...
        new_key = new_key.replace("embeddings.position_embeddings", "pos_embed")
        new_key = new_key.replace("embeddings.patch_embeddings.projection.weight", "patch_embed.proj.weight")
        new_key = new_key.replace("embeddings.patch_embeddings.projection.bias", "patch_embed.proj.bias")
        
        # Transformer Blocks 변환
        match = re.match(r'encoder\.layer\.(\d+)\.(.*)', new_key)
        if match:
        ...
        # LayerNorm 변환
        new_key = new_key.replace("layernorm.weight", "norm.weight")
        new_key = new_key.replace("layernorm.bias", "norm.bias")
